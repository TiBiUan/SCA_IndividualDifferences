...
function [h, s] = plotcov3(mu, Sigma, draw_to_these_axes, COL, ALPHA,pro,varargin)
...
[p, ...
 n, ...
 plot_opts, ...
 surf_opts] = process_options(varargin, 'conf',  0.682,...%0.9, ...
					'num-pts', 20, ...
			                'plot-opts', {}, ...
			                'surf-opts', {});
h = [];
...
s = surf(draw_to_these_axes, reshape(z(1, :), [(n + 1) (n + 1)]), ...
	 reshape(z(2, :), [(n + 1) (n + 1)]), ...
	 reshape(z(3, :), [(n + 1) (n + 1)]), ...
	 surf_opts{:},'EdgeColor','None');
 
 set(s,'FaceColor',COL,'FaceAlpha',ALPHA);
